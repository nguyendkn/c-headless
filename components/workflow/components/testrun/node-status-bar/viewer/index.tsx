...
const TreeNode: React.FC<TreeNodeProps> = ({
  ...
  const renderPrimitiveValue = (val: any) => {
    if (val === null)
      return (
        <span className={cn(styles.primitiveValue, styles.null)}>null</span>
      );
    if (val === undefined)
      return (
        <span className={cn(styles.primitiveValue, styles.undefined)}>
          undefined
        </span>
      );
    ...
    switch (typeof val) {
      case 'string':
        return (
          <span>
            ...
            <span
              className={cn(styles.primitiveValue, styles.string)}
      ...
      case 'number':
        return (
          <span
            className={cn(styles.primitiveValue, styles.number)}
      ...
      case 'boolean':
        return (
          <span
            className={cn(styles.primitiveValue, styles.boolean)}
      ...
      case 'object':
        ...
        if (Array.isArray(val)) {
          return (
            <span
              className={styles.primitiveValue}
...
export const DataStructureViewer: React.FC<DataStructureViewerProps> = ({
  data,
  level: _level = 0,
}) => {
